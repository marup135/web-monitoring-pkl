...
import { Moon, Globe, Info, LogOut, ChevronRight, User, Image as ImageIcon, Upload, Trash2, Loader2, Key, Check, Palette, Shield, Settings, ArrowLeft, HelpCircle, Clock } from 'lucide-react';
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ 
  ...
  const { state, currentUser, logout, updateCurrentUserName, updateCurrentUserBackground } = usePKL();
  ...
  const renderProfileTab = () => (
    <div className="flex flex-col gap-8 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-[#1E293B] rounded-2xl shadow-sm border border-slate-100 dark:border-gray-800 p-6">
        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-gray-800">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-primary overflow-hidden border-2 border-white dark:border-gray-800 shadow-sm shrink-0">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-800 dark:text-white">{currentUser?.name}</h2>
            <span className="inline-block px-2.5 py-1 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 font-bold text-[10px] uppercase tracking-wider rounded-md border border-blue-100 dark:border-blue-500/20 mt-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/50">
            <span className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider block mb-1">{t('nameLabel')}</span>
            <span className="font-semibold text-slate-700 dark:text-gray-200">{currentUser?.name}</span>
          </div>
          <div className="bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/50">
            <span className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider block mb-1">NIS / NISN</span>
            <span className="font-semibold text-slate-700 dark:text-gray-200">{state?.nisn || currentUser?.nisn || '-'}</span>
          </div>
          <div className="bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/50">
            <span className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider block mb-1">{t('schoolOrigin')} / {t('companyOrigin')}</span>
          ...
          <div className="bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/50">
            <span className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider block mb-1">{t('internCompany')}</span>
            <span className="font-semibold text-slate-700 dark:text-gray-200">{state?.companyName || currentUser?.companyName || currentUser?.company || '-'}</span>
          </div>
          <div className="bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/50 md:col-span-2">
            <span className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider block mb-1">{t('advisorLabel')}</span>
            <div className="flex flex-col md:flex-row gap-4 mt-2">
              <div className="flex-1">
                <span className="text-[10px] text-slate-500 block mb-0.5">{t('internalAdvisor')}</span>
                <span className="font-semibold text-sm text-slate-700 dark:text-gray-200">{state?.advisorName || '-'}</span>
              </div>
              <div className="flex-1">
                <span className="text-[10px] text-slate-500 block mb-0.5">{t('externalAdvisor')}</span>
                <span className="font-semibold text-sm text-slate-700 dark:text-gray-200">{state?.mentorName || '-'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-gray-800">
          {!isEditingProfile ? (
            <button onClick={() => setIsEditingProfile(true)} className="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white rounded-xl text-sm font-bold shadow-sm transition-all duration-200">
              {t('editProfile')}
            </button>
          ) : (
            <form onSubmit={handleUpdateProfile} className="flex flex-col gap-4 bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-gray-600 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-semibold"
                />
              </div>
              <div className="flex gap-3 justify-end">
                ...
                <button type="submit" className="px-5 py-2 bg-primary hover:bg-primary-hover text-white rounded-xl text-sm font-bold shadow-sm transition-all duration-200">
                  {t('save')}
                </button>
              </div>
            </form>
